...
public class Ban implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		if(cs instanceof Player) {
			Player p = (Player) cs;
			if(core.getGroups().hasPermission(p, "elerion.command.ban")) {
				if(args.length < 2) {
					p.sendMessage(core.getPrefix() + "§cIncorrect usage. Please use /ban <player> <reason>!");
				}else{
					...
					if(target != null) {
						try {
							core.getPunisher().issuePunishment(p, target, PunishmentType.BAN, 0, null, reason);
						...
						if(core.getPlayerAccount().existsPlayer(t.getUniqueId().toString())) {
							try {
								core.getPunisher().issueOfflinePunishment(p, t.getUniqueId().toString(), t.getName(), PunishmentType.BAN, 0, null, reason);
							} catch (SQLException e) {
								e.printStackTrace();
							}
						}else{
